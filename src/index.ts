...
game.loop(() => {
  if (!isGameOver) {
    time.innerHTML = `${Math.max(0, game.action - 1)}`;
  }
  if (game.score !== scoreNum) {
    scoreNum = game.score|0;
    highScore = Math.max(highScore, scoreNum);
    score.innerHTML = `${scoreNum} souls`;
  }
  if (!isGameOver && !game.heroAlive) {
    gameover.style.display = 'block';
    gameover.innerHTML = `After reaping ${scoreNum} souls, you rested eternally. <br/> Click to try again.`;
    isGameOver = true;
  }
});
